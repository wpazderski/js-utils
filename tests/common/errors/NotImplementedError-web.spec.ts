        ...
        test.describe("instance", () => {
            test("should be an instance of Error", async ({ page }) => {
                const isOk = await page.evaluate(() => {
                    const error = new window.jsUtils.common.errors.NotImplementedError("Test 123");
            ...
            test("should have message that includes the subject", async ({ page }) => {
                ...
                const isOk = await page.evaluate(
                    ...
                    ([subject]) => {
                        const error = new window.jsUtils.common.errors.NotImplementedError(subject);
                        window.playwrightUtils.expect(error.message).toContain(subject);
                        return true;
                    },
                    [subject] as [string],
                );
