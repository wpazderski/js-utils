        ...
        test.describe("instance", () => {
            test("should be an instance of Error", async ({ page }) => {
                const isOk = await page.evaluate(() => {
                    const error = new window.jsUtils.common.collections.errors.RegistryEntryAlreadyRegisteredError("TestEntry123");
            ...
            test("should have message that includes the entry name", async ({ page }) => {
                ...
                const isOk = await page.evaluate(
                    ...
                    ([entryName]) => {
                        const error = new window.jsUtils.common.collections.errors.RegistryEntryAlreadyRegisteredError(entryName);
